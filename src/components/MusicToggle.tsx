...
function IndicatorOn({ isActive }: { isActive: boolean }) {
  return (
    <div className={`relative shrink-0 size-3 transition-opacity duration-300 ${isActive ? 'opacity-100' : 'opacity-0'}`} data-name="Indicator on">
      <Indicator />
    </div>
  );
}
...
function IndicatorOff({ isActive }: { isActive: boolean }) {
  return (
    <div className={`relative shrink-0 size-3 transition-opacity duration-300 ${isActive ? 'opacity-100' : 'opacity-0'}`} data-name="Indicator off">
      <Indicator1 />
    </div>
  );
}
...
function Background({ enabled }: { enabled: boolean }) {
  return (
    <div className={`absolute box-border content-stretch flex h-10 items-center justify-between left-0 overflow-clip pl-4 pr-0 py-0 rounded-[99px] top-0 w-20 transition-colors duration-300 ${
...
function Handle({ enabled }: { enabled: boolean }) {
  return (
    <div className={`relative shrink-0 size-9 transition-transform duration-300 ${
...
export function MusicToggle({ enabled, onToggle }: MusicToggleProps) {
  ...
  // 初始化 audio 对象
  if (!audioRef.current) {
    audioRef.current = new Audio("https://screenapp.io/app/#/shared/5_nUuwh2_-"); // 替换成你的音乐文件路径
